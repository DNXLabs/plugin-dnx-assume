...
@click.option('-i', '--aws_account_id', 'aws_account_id', default=None, type=str, help='AWS account id to assume.')
def dnx_assume(aws_role, aws_account_id):
    aws_role = aws_role or get_config_value('plugins.dnx-assume.parameters.aws-role')
    aws_account_id = aws_account_id or get_config_value('plugins.dnx-assume.parameters.aws-account-id')
